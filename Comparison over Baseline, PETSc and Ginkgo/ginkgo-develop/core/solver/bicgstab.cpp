                                           ...
                                           VectorType* dense_x) const
{
    ...
    while (true) {
        ...
        time_precond += (t4.tv_sec - t3.tv_sec) * 1000.0 + (t4.tv_usec - t3.tv_usec) / 1000.0;
        // v = A * y
    ...
    double time_bicg = (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0;
    printf("time_bicg=%lf,time_precond=%lf,iter=%d,",time_bicg,time_precond,iter);
    char *s1 = (char *)malloc(sizeof(char) * 100);
    sprintf(s1, "iter=%d,time_bicg=%lf,time_precond=%lf,",iter,time_bicg,time_precond);
    FILE *file1 = fopen("ginkgo_bicg_a100_iter10.csv", "a");
    fwrite(s1, strlen(s1), 1, file1);
    free(s1);
    fclose(file1);
}
