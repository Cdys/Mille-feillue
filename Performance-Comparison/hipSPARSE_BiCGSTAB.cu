             ...
             int nnzR, char *filename, int n) // bicg
{
    ...
    // hipsparseDnVecDescr_t vecX1, vecY1;
    void *dBuffer = NULL;
    size_t bufferSize = 0;
    ...
    // hipsparseCreateDnVec(&vecX, n, k_pg, HIP_R_64F);
    // hipsparseCreateDnVec(&vecY, n, k_vg, HIP_R_64F);

    // hipsparseCreateDnVec(&vecX1, n, k_sg, HIP_R_64F);
    // hipsparseCreateDnVec(&vecY1, n, k_tg, HIP_R_64F);
    double alpha = 1.0f;
    double beta = 0.0f;
    ...
    for(int i = 0; i < 1; ++i)
    {
        ...
        hipsparseDcsrmv(handle,HIPSPARSE_OPERATION_NON_TRANSPOSE,
                n,n,nnzR,&alpha,
                descrA,dA_values,dA_csrOffsets,dA_columns,k_pg,&beta,k_vg); 
        //hipblasDdot(cublasHandle, n, k_rh, 1, k_vg, 1, k_pra);
    }
    ...
    for (itr = 0; itr < 5; itr++)
    {
        ...
        hipMemset(k_vg, 0, n * sizeof(double));
        hipMemset(k_tg, 0, n * sizeof(double));
        hipDeviceSynchronize();
        ...
        //hipMemset(k_pra, 0, sizeof(double));
        ...
        // hipsparseSpMV(handle, HIPSPARSE_OPERATION_NON_TRANSPOSE,
        //                 &alpha, matA, vecX1, &beta, vecY1, COMPUTE_TYPE,
        //                     HIPSPARSE_MV_ALG_DEFAULT, dBuffer);
        hipsparseDcsrmv(handle,HIPSPARSE_OPERATION_NON_TRANSPOSE,
                                n,n,nnzR,&alpha,
                                descrA,dA_values,dA_csrOffsets,dA_columns,k_sg,&beta,k_tg);
        hipDeviceSynchronize();
        ...
        //hipMemset(k_tmp1, 0, sizeof(double));
        // sdot2_2<<<GridDim, BlockDim>>>(k_tg, k_sg, k_tmp1, n);
        hipblasDdot(cublasHandle, n, k_tg, 1, k_sg, 1, k_tmp1);/////
        //hipMemset(k_tmp2, 0, sizeof(double));
        // sdot2_2<<<GridDim, BlockDim>>>(k_tg, k_tg, k_tmp2, n);
        hipblasDdot(cublasHandle, n, k_tg, 1, k_tg, 1, k_tmp2);/////
        hipDeviceSynchronize();
        gettimeofday(&t8, NULL);
        time_dot += (t8.tv_sec - t7.tv_sec) * 1000.0 + (t8.tv_usec - t7.tv_usec) / 1000.0;
        //prc=tmp1/tmp2;
    ...
    sprintf(s, "%d,%.3f,%.3f,%d,%e,%lf,%e\n", itr, time_spmv, time_bicg, nnzR,l2_norm,time_dot,norm);
    FILE *file1 = fopen("bicg_cusparse_amd_mi200_5.csv", "a");
    if (file1 == NULL)
    {
        printf("open error!\n");
        return 0;
