...
// PetscCall(VecNorm(R, NORM_2, &dp)); /*    dp <- r'*r = e'*A'*A*e            */
  ...
  do {
    ...
    gettimeofday(&t8, NULL);
    time_spmv += (t8.tv_sec - t7.tv_sec) * 1000.0 + (t8.tv_usec - t7.tv_usec) / 1000.0;
    //printf("d4\n");
    ...
    if (ksp->normtype == KSP_NORM_PRECONDITIONED && ksp->chknorm < i + 2) {
     ...
     // time_pc += (t10.tv_sec - t9.tv_sec) * 1000.0 + (t10.tv_usec - t9.tv_usec) / 1000.0;
      KSPCheckNorm(ksp, dp);
    ...
    if ((ksp->normtype != KSP_NORM_NATURAL) || (ksp->chknorm >= i + 2)) {
      PetscCall(VecXDot(Z, R, &beta)); /*     beta <- z'*r                     */
      //PetscCall(VecXDot(R, R, &beta)); /*     beta <- z'*r                     */
      KSPCheckDot(ksp, beta);
    }
    i++;
  } while (i < ksp->max_it);
  gettimeofday(&t6, NULL);
  double time_cg = (t6.tv_sec - t5.tv_sec) * 1000.0 + (t6.tv_usec - t5.tv_usec) / 1000.0;
  printf("time_cg=%lfms time_pre=%lfms\n",time_cg,time_pre);
  // if(i>=1)
  // {
  char *s = (char *)malloc(sizeof(char) * 100);
  sprintf(s, "iter=%d,time_solve=%.3lf,time_pre=%.3lf,time_spmv=%.3lf\n", i, time_cg,time_pre,time_spmv);
  FILE *file2 = fopen("petsc_jpcg_a100_residual_1e-6.csv", "a");
