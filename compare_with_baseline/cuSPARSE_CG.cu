...
// #pragma omp parallel for
    ...
    while (iterations < 100) //&& snew > epsilon * epsilon * s0)
    {
        // q = Ad
    ...
    sprintf(s, "iter=%d,time_spmv=%.3f,time_cg=%.3f,nnzR=%d,l2_norm=%lf,time_format=%lf\n", iterations, time_spmv, time_cg, nnzR, l2_norm,time_format);
    FILE *file1 = fopen("cg_initial_a100.csv", "a");
    if (file1 == NULL)
    {
        printf("open error!\n");
        return;
