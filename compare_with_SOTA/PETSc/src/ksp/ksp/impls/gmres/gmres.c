...
PetscErrorCode KSPGMRESCycle(PetscInt *itcount, KSP ksp)
{
  ...
  *GRS(0) = gmres->rnorm0 = res;
  ...
  time_new+=(t10.tv_sec - t9.tv_sec) * 1000.0 + (t10.tv_usec - t9.tv_usec) / 1000.0;
...
PetscErrorCode KSPSolve_GMRES(KSP ksp)
{
  ...
  while (!ksp->reason || (ksp->rnorm == -1 && ksp->reason == KSP_DIVERGED_PC_FAILED)) {
    ...
    //time_cycle+=(t8.tv_sec - t7.tv_sec) * 1000.0 + (t8.tv_usec - t7.tv_usec) / 1000.0;
    ...
    //time_hess+=(t_e.tv_sec - t_s.tv_sec) * 1000.0 + (t_e.tv_usec - t_s.tv_usec) / 1000.0;
    itcount += its;
  ...
  double time_gmres = (t6.tv_sec - t5.tv_sec) * 1000.0 + (t6.tv_usec - t5.tv_usec) / 1000.0;
  printf("time_gmres=%.3f\n",time_gmres);
  char *s = (char *)malloc(sizeof(char) * 100);
  sprintf(s, "iter=%d,time_solve=%.3lf,time_pre=%.3lf\n", itcount, time_new,time_gmres-time_new);
  FILE *file2 = fopen("petsc_gmres_a100_residual.csv", "a");
